...
if($_POST){
    ...
    if(empty($nama_user)){
        ...
        $insert=mysqli_query($conn,"insert into user (id_user, nama_user, username, password, level,alamat,telp) value ('".$id_user."','".$nama_user."','".$username."','".$password."','".$level."','".$alamat."','".$telp."')") or die(mysqli_error($conn));
        if($insert){
